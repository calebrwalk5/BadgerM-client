...
void driver_buttons(ITEM *item) {
	...
	if (strcmp(name, "OK") == 0) {
		mvprintw(LINES-2, 1, "[*] Starting BadgerM:\t");

		for (i = 0; i < form->maxfield; i++) {
			printw("%s", field_buffer(fields[i], 0));
			FILE* fp = fopen("/crypt/pw.txt", "w+");
			fputs(fields[i], fp);
			fclose(fp);
			connect(fields);
		...
		mvprintw(LINES-2, 1, "[*] QUIT clicked, 'F2' to quit\n");
...
int main() {
	...
	char *requests[] = { "Password:", "", "Username:", "" };
	int ch;
	int i;
...
int connect() {
	system("echo connect function is working");
}
